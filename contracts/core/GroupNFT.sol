 ...
 * 
 * -----
 * Self-Managed groups are to allow members to vote on groups's parameters & policies (govern)
 *  - Ban
 *  - Pin
 *  - ?
 * 
 */
contract GroupNFT is LensNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    // We create the GroupNFT with the pre-computed HUB address before deploying the hub.
    ...
    function initialize(
        ...
        // emit Events.FollowNFTInitialized(profileId, block.timestamp);    //TBD
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // return ILensHub(HUB).getFollowNFTURI(_profileId);       //TODO!! - Individual GroupNFT URI (From Self)
    }
    ...
    function _beforeTokenTransfer(
        ...
        // ILensHub(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);     //TBD - Individual GroupNFT Transfer Event
    ...
    function _moveDelegate(
        ...
        unchecked {
            if (from != address(0)) {
                ...
                // emit Events.FollowNFTDelegatedPowerChanged(from, newValue, block.timestamp);     //TBD - Individual GroupNFT Delegated Power Changed Event
            }
            ...
            if (to != address(0)) {
                ...
                // emit Events.FollowNFTDelegatedPowerChanged(to, newValue, block.timestamp);   //TBD
            } else {
